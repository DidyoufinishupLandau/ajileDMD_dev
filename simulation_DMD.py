...
class SimulationCompressingImage:
    ...
    def simulate_random_pattern(self, sparsity=0.50, sampling_rate=1, noise_rate=0):
        ...
        image = np.sum(np.array(image), axis=0)/(int(self.width * self.height * sampling_rate))
        return image

    def simulate_hadmard_pattern(self, sampling_rate=1, noise_rate=0):
        DP = DmdPattern("hadamard", self.width, self.height)
        mask_matrix_one = DP.execute(two_dimension=True)
        image = []
...
simulated_image = s.simulate_hadmard_pattern(sampling_rate=0.9, noise_rate=0.00)
plot_pixel(image)
plot_pixel(simulated_image)

a = PSNR(image, simulated_image)
psnr_value = a.execute()
print(psnr_value)